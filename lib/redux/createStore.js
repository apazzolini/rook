...
var apiLoadingReducer = _interopRequireWildcard(_api);
...
function createRootReducer(reducers) {
  var rootReducer = {
    ...
    apiRequest: (0, _createReducer2.default)(apiLoadingReducer)
  };
  ...
  try {
    for (var _iterator = (0, _getIterator3.default)(_Object$entries), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      rootReducer[k] = (0, _createReducer2.default)(v);
    }
  } catch (err) {
...
function create(apiClient, providedMiddleware, history, data) {
  ...
  if (__DEVELOPMENT__ && __CLIENT__ && __DEVTOOLS__) {
    ...
    finalCreateStore = devtools.compose(middleware)(_redux.createStore);
  } else {
