    ...
    if (__DISABLE_SSR__) {
      ...
      (0, _reactRouter.match)({ history: history, routes: getRoutes(store), location: request.url.path }, function (error, redirectLocation, renderProps) {
        if (redirectLocation) {
          ...
          try {
            (0, _fetchComponentData2.default)(renderProps.components, store.getState, store.dispatch, renderProps.location, renderProps.params).then(function () {
              var component = rootComponent.createForServer(store, renderProps);
              var httpStatus = (0, _getRouteHttpStatus2.default)(renderProps.routes) || 200;
